  ...
  try {
    ...
    if (session.participants) {
      if (session.participants.toString() === userId.toString()) {
        return res
          .status(200)
          .json({ session, msg: "user already a participant" });
      }
    ...
    try {
      ...
      await session.save();
      console.error("Stream error while adding participant:", streamError);
      return res
        .status(502)
        .json({ msg: "failed to add participant to stream" });
    }
